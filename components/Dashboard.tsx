...
const QuickLink: React.FC<QuickLinkProps> = ({ icon, title, onClick }) => (
    <button onClick={onClick} className="flex items-center p-3 -m-3 rounded-lg hover:bg-gray-100 transition-colors duration-200 w-full text-left">
        ...
        <svg className="w-5 h-5 text-gray-400 ml-auto" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M9 5l7 7-7 7"></path></svg>
    </button>
);
...
const Dashboard: React.FC<DashboardProps> = ({ products, orders, expenses, incomes, customers, setPage }) => {
    ...
    return (
        <div className="bg-gray-50/50">
            <div className="mb-8">
                <h1 className="text-3xl font-bold text-gray-800">Selamat Datang, Owner!</h1>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
                <div className="lg:col-span-2 bg-white rounded-xl shadow-sm">
                    <div className="p-6 flex justify-between items-center border-b border-gray-200">
                        <h3 className="text-lg font-semibold text-gray-800">Transaksi Terakhir Hari Ini</h3>
                        ...
                        {todayStats.recentTransactions.length > 0 ? (
                            <table className="min-w-full">
                                <thead className="bg-gray-50">
                                    <tr>
                                        <th className="py-3 px-6 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">No. Bon</th>
                                ...
                                <tbody className="divide-y divide-gray-200">
                                    {todayStats.recentTransactions.map((order) => (
                                        <tr key={order.id} className="hover:bg-gray-50">
                                            <td className="py-4 px-6 text-sm text-gray-700 font-medium whitespace-nowrap">{order.bon}</td>
                                            <td className="py-4 px-6 text-sm text-gray-500 whitespace-nowrap">{order.date.split(', ')[1]}</td>
                                            <td className="py-4 px-6 text-sm text-gray-900 whitespace-nowrap">{order.customer}</td>
                                            <td className="py-4 px-6 text-sm text-gray-800 text-right font-semibold whitespace-nowrap">{formatRupiah(order.grandTotal)}</td>
                                        </tr>
                                    ))}
                                </tbody>
                            </table>
                            ...
                            <div className="text-center py-16">
                                ...
                                <h3 className="text-lg font-semibold text-gray-700">Belum Ada Transaksi Hari Ini</h3>
                            </div>
                        )}
                    </div>
                </div>
                ...
                <div className="space-y-8">
                     <div className="bg-white rounded-xl shadow-sm p-6">
                        ...
                        <div className="space-y-2">
                            ...
                            <QuickLink title="Lihat Laporan" onClick={() => setPage('Keuangan')} icon={<svg className="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M9 19v-6a2 2 0 00-2-2H5a2 2 0 00-2 2v6a2 2 0 002 2h2a2 2 0 002-2zm0 0V9a2 2 0 012-2h2a2 2 0 012 2v10m-6 0a2 2 0 002 2h2a2 2 0 002-2m0 0V5a2 2 0 012-2h2a2 2 0 012 2v14a2 2 0 01-2 2h-2a2 2 0 01-2-2z"></path></svg>} />
                        </div>
                    </div>
                    <div className="bg-white rounded-xl shadow-sm">
                        <div className="p-6 border-b border-gray-200">
                             <h3 className="text-lg font-semibold text-gray-800">Stok Hampir Habis</h3>
                        </div>
                        {todayStats.lowStockProducts.length > 0 ? (
                             <ul className="divide-y divide-gray-200 p-6 max-h-60 overflow-y-auto">
                            ...
                            <p className="text-sm text-gray-500 text-center py-10 px-6">Semua stok aman!</p>
                        )}
                    </div>
                </div>
            </div>
