    ...
    return (
        <div 
            className={`rounded-xl border border-gray-200 overflow-hidden flex flex-col group relative transition-all duration-300 ${
                isOutOfStock ? 'cursor-not-allowed bg-gray-50' : 'cursor-pointer hover:shadow-lg hover:border-blue-400 hover:-translate-y-1'
            }`}
            ...
            <div className="relative">
                <div className="aspect-[4/3] bg-gray-100">
                     {product.imageUrl ? (
                        ...
                        <div className={`w-full h-full flex items-center justify-center bg-gray-200 ${isOutOfStock ? 'grayscale' : ''}`}>
                ...
                {isOutOfStock && (
                     <div className="absolute inset-0 bg-white/70 flex items-center justify-center">
                        <span className="font-bold text-gray-600">STOK HABIS</span>
                    </div>
                )}
            </div>
            <div className="p-4 text-left flex-grow flex flex-col justify-between bg-white">
                <div>
                    <h3 className="text-md font-semibold text-gray-800 leading-tight truncate flex items-center gap-1.5">
                        {product.name}
                        {hasVariations && (
