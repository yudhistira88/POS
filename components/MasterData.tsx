            ...
            <div className="bg-white rounded-xl shadow-lg">
                ...
                <div className="border-b border-gray-200">
                    <nav className="-mb-px flex space-x-6 px-6" aria-label="Tabs">
                        {['Produk', 'Bundling', 'Kategori'].map(tabName => (
                            <button
                                key={tabName}
                                onClick={() => setActiveTab(tabName)}
                                className={`${activeTab === tabName ? 'border-blue-500 text-blue-600' : 'border-transparent text-gray-500 hover:text-gray-700'} whitespace-nowrap py-4 px-1 border-b-2 font-medium text-sm flex items-center gap-2`}
                            >
                                {tabName}
                                <span className={`px-2 py-0.5 rounded-full text-xs ${activeTab === tabName ? 'bg-blue-100 text-blue-600' : 'bg-gray-100 text-gray-600'}`}>
                                    {tabName === 'Produk' ? products.length : tabName === 'Bundling' ? bundles.length : categories.length}
                                </span>
                            </button>
                        ))}
                    </nav>
