...
const ImageIcon = () => <svg className="w-8 h-8 text-gray-400" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M4 16l4.586-4.586a2 2 0 012.828 0L16 16m-2-2l1.586-1.586a2 2 0 012.828 0L20 14"></path></svg>;
...
const ProductModal: React.FC<ProductModalProps> = ({ product, onClose, onSave }) => {
    ...
    return (
        <div 
            ...
            role="dialog"
        >
            <div className="bg-white rounded-xl shadow-xl w-full max-w-3xl transform transition-all">
                <form onSubmit={handleSubmit}>
                    ...
                    <div className="p-6 max-h-[70vh] overflow-y-auto grid grid-cols-1 md:grid-cols-2 gap-x-6 gap-y-5">
                         ...
                         <FormSection title="Harga & Stok">
                            <div className="grid grid-cols-2 gap-4">
                                <FormInput id="price" label="Harga Jual" required>
                                    <div className="relative"><div className="pointer-events-none absolute inset-y-0 left-0 flex items-center pl-3"><span className="text-gray-500 sm:text-sm">Rp</span></div><input type="number" name="price" id="price" value={formData.price} onChange={handleChange} required className={`${inputClass} pl-9`} placeholder="15000" /></div>
                                </FormInput>
                            ...
                            <div className="pt-2">
                                <p className="text-sm font-medium text-gray-700 mb-1">Diskon Produk (Opsional)</p>
                                <div className="grid grid-cols-2 gap-4">
                                    ...
                                    <div className="relative"><div className="pointer-events-none absolute inset-y-0 left-0 flex items-center pl-3"><span className="text-gray-500 sm:text-sm">{formData.discount?.type === 'fixed' ? 'Rp' : '%'}</span></div><input type="number" name="value" value={formData.discount?.value || 0} onChange={handleDiscountChange} className={`${inputClass} pl-8`} placeholder="0" /></div>
                                </div>
                            </div>
                        </FormSection>
                        ...
                        <FormSection title="Variasi Produk (Opsional)">
                                    ...
                                    <div className="pl-2 space-y-2">
                                        {group.options.map((option, optionIndex) => (
                                            <div key={optionIndex} className="flex items-center gap-2">
                                                <input type="text" value={option.name} onChange={(e) => handleOptionChange(groupIndex, optionIndex, 'name', e.target.value)} placeholder="Nama Opsi" className="flex-grow border-gray-300 rounded-md text-sm p-1" />
                                                <input type="number" value={option.priceAdjustment} onChange={(e) => handleOptionChange(groupIndex, optionIndex, 'priceAdjustment', Number(e.target.value))} placeholder="+/- Harga" className="w-24 border-gray-300 rounded-md text-sm p-1" />
                                                <button type="button" onClick={() => handleRemoveOption(groupIndex, optionIndex)} className="text-gray-400 hover:text-red-600 p-0.5"><svg className="w-3 h-3" fill="currentColor" viewBox="0 0 20 20"><path fillRule="evenodd" d="M4.293 4.293a1 1 0 011.414 0L10 8.586l4.293-4.293a1 1 0 111.414 1.414L11.414 10l4.293 4.293a1 1 0 01-1.414 1.414L10 11.414l-4.293 4.293a1 1 0 01-1.414-1.414L8.586 10 4.293 5.707a1 1 0 010-1.414z" clipRule="evenodd" /></svg></button>
                                            </div>
                                        ))}
                            ...
                            <button type="button" onClick={handleAddVariationGroup} className="w-full text-sm font-semibold text-blue-600 border-2 border-dashed border-gray-300 rounded-lg py-2 hover:bg-blue-50">+ Tambah Grup Variasi</button>
                        </FormSection>

                        <div className="md:col-span-2">
                             <div className="flex justify-between items-center mb-1">
                                <label htmlFor="description" className="block text-sm font-medium text-gray-700">Deskripsi & Keterangan</label>
                        ...
                        <div className="md:col-span-2">
                            ...
                            <div className="mt-1 flex items-start gap-4">
                                <div className="w-32 h-32 rounded-lg bg-gray-100 border border-dashed flex items-center justify-center overflow-hidden flex-shrink-0">
                                     {formData.imageUrl && !imageError ? (
                                        <img src={formData.imageUrl} alt="Pratinjau Produk" className="w-full h-full object-cover" onError={() => setImageError(true)} />
                                    ) : (
                                        <div className="text-center text-gray-500 p-2"><ImageIcon /><p className="text-xs mt-1">Pratinjau</p></div>
                                    )}
                                </div>
                                <div className="flex-grow">
                                    <input type="file" ref={fileInputRef} onChange={handleFileChange} accept="image/png, image/jpeg" hidden/>
                                    <button type="button" onClick={handleImageUploadClick} className="w-full text-center px-4 py-2 border border-gray-300 rounded-md shadow-sm text-sm font-medium text-gray-700 bg-white hover:bg-gray-50">Upload dari Komputer</button>
                                     <div className="relative my-2"><div className="absolute inset-0 flex items-center" aria-hidden="true"><div className="w-full border-t border-gray-300" /></div><div className="relative flex justify-center text-sm"><span className="bg-white px-2 text-gray-500">atau</span></div></div>
                                    <input type="text" name="imageUrl" value={formData.imageUrl.startsWith('data:') ? '' : formData.imageUrl} onChange={handleChange} className={inputClass} placeholder="Tempel URL gambar disini" />
                                </div>
                            </div>
                        </div>
                    ...
                    <div className="bg-gray-50 px-6 py-4 flex flex-row-reverse rounded-b-xl">
                        ...
                        <button type="button" onClick={onClose} className="inline-flex justify-center rounded-lg border border-gray-300 shadow-sm px-5 py-2.5 bg-white text-base font-medium text-gray-700 hover:bg-gray-50 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-indigo-500 w-auto sm:text-sm">Batal</button>
                    </div>
                </form>
            </div>
        </div>
